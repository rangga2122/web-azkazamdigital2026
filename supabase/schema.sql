...
create table public.site_settings (
  ...
  pakasir_enabled boolean not null default false,
  pakasir_mode text not null default 'sandbox' check (pakasir_mode in ('sandbox', 'live')),
  pakasir_project_slug text,
  pakasir_api_key text,
  pakasir_webhook_url text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table public.orders (
  ...
  gateway_total_payment numeric(14,2),
  gateway_payment_number text,
  gateway_expired_at timestamptz,
